    ...
    ffmpeg_("ffmpeg"), stipdet_("~/stip-2.0-linux/bin/stipdet")
{
  ...
  classify_client_ = nh_->serviceClient<activity_recognition_server::classify>("feature_directory", 1);

  // initialize a preview window
  cv::namedWindow("moverio");
  cv::startWindowThread();
  cv::Mat img = cv::imread("/home/kazuto/catkin_ws/src/activity_recognition/activity_recognition_server/moverio.jpg", 1);
  cv::imshow("moverio", img);

  // ffmpeg operation
  // input source
  ffmpeg_ += " -i ";
  ffmpeg_ += "~/catkin_ws/src/activity_recognition/activity_recognition_server/video/moverio.avi";
  // output codec
  ffmpeg_ += " -vcodec ";
  ffmpeg_ += "libxvid";
  // output size
  ...
  // output path
  ffmpeg_ += " -y ";
  ffmpeg_ += "~/catkin_ws/src/activity_recognition/activity_recognition_server/video/moverio_converted.avi";

  // STIP detection
  // input list file
  stipdet_ += " -i ";
  stipdet_ += "~/catkin_ws/src/activity_recognition/activity_recognition_server/video/video_list.txt";
  // input video path
  stipdet_ += " -vpath ";
  stipdet_ += "~/catkin_ws/src/activity_recognition/activity_recognition_server/video/";
  // output path
  stipdet_ += " -o ";
  stipdet_ += "~/catkin_ws/src/activity_recognition/activity_recognition_server/moverio.txt";
}
...
{
  std_msgs::String msg;
  activity_recognition_server::classify srv;
